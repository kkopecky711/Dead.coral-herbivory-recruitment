---
title: "Dead skeletons-herbivory-recruitment"
author: "Kai Kopecky"
date: "`r Sys.Date()`"
...
## Grazing scars
```{r Data cleaning and statistics}

# Load data for grazing scars etween dead skeletons
...
# Load dataset for removal of turf algae in herbivory assays
...
# Calculate means with replicates
turf.summary <- turf %>% 
...
# Load raw data for macoralgal biomasses on herbivroy assays
algal_weights.raw <- read_csv("Data/Herbivory assays data_master.csv") %>% 
  clean_names()

# Create columns for proportional change in mass and algal preference
algal_weights.prop <- algal_weights.raw %>% 
  ...
  mutate(preference = case_when(taxa == "Amansia" ~ "More preferred",
         ...
         treatment = case_when(treatment == "Dead Coral" ~ "Dead coral",
                               ...
                               treatment == "Open" ~ "Open"),
         prop_removal = if_else(prop_removal <= 0, 0.0000001, prop_removal)) # Adds very small constant to account for zeroes
...
```{r Statistics for macroalgae assays}

## Log-difference in removal
# Calculate relative differences for each assay pair
...
## Algae accummulation experiment
```{r Algae data wrangling and cleaning}
# Load dataset for accummulation of macroalgae on experimental reef patches
...
# Calculate mean algae cover by treatment and time point
...
# Calculate proportions of preferred vs less preferred macroalgae at each time point (in dead skeleton treatment only)
...
# Create summary table of total algal cover at each replicate patch reef
...
# Plot histogram of proportion of preferred and less preferred algae 
...
## Generalized linear mixed effects model of macroalgae accumulation ~ skeleton treatment and time
...
algae_totals.short <- algae_totals %>% 
  ...
  mutate(days_since_start = as.factor(days_since_start),
         prop_const = total_macroalgae.prop + 0.001) # add small constant to ensure values are > 0 for model analysis
...
algae_cover.glmm <- glmmTMB(prop_const ~ treatment*days_since_start + (1|bommie_number), 
...
```{r Visualization of recruit densities ~ macroalgae cover}
...
# Calculate total for coral cover in replicate quads
...
#### Analysis of macroalgal cover before and after cyclone and bleaching disturbances
```{r Macroalgae cover before and after disturbances}

# Read in and clean MCR algae data, removing non-macoralgal taxa
...
# Ensure equal number of replicates for each year 
mcr_algae.quads.n <- mcr_algae.quads %>% 
  summarize(n = n())
...
mcr_algae.summary <- mcr_algae.quads %>% 
  ...
  group_by(year) %>% 
  summarize(perc_cover.mean = mean(perc_cover),
            perc_cover.se = sd(perc_cover)/sqrt(n()))

# Create individual dataframes of macroalgae cover for 4 years before and after each disturbance type
algae.before_oli <- mcr_algae.summary %>% 
  filter(year == "2005" | year == "2007" | year == "2008" | year ==  "2009") %>% 
  mutate(dist_period = "Before",
         dist_type = "Cyclone")

algae.after_oli <- mcr_algae.summary %>% 
  filter(year == "2011" | year == "2012" | year == "2013" | year == "2014") %>% 
  mutate(dist_period = "After",
         dist_type = "Cyclone")

algae.before_bleaching <- mcr_algae.summary %>% 
  filter(year == "2015" | year == "2016" | year == "2017" | year == "2018") %>% 
  mutate(dist_period = "Before",
         dist_type = "Bleaching")

algae.after_bleaching <- mcr_algae.summary %>% 
  filter(year == "2020" | year == "2022" | year == "2023") %>%
  mutate(dist_period = "After",
         dist_type = "Bleaching")
...
algae.before_after.summary <- algae.before_after %>% 
  ...
  summarize(mean = mean(perc_cover.mean),
            SE = sd(perc_cover.mean)/sqrt(n())) %>% 
...
algae.before_after <- algae.before_after %>% 
  mutate(cover_prop = perc_cover.mean/100)

algae.before_after.glmm <- glmmTMB(cover_prop ~ dist_period*dist_type + (1|year),
                                   family = beta_family(link = "logit"),
...
## Recruit performance analyses
```{r Recruit data cleaning and wrangling}

#Load and clean dataframe for coral recruits
recruits_raw <- read_csv("Data/Beacons recruit performance experiment - Beacons recruit performance_master.csv") %>%
  clean_names() 

# Remove corals with no final measurements, calculate volume, proportional volume change, and proportional mass change 
recruits <- recruits_raw %>% 
  select(-c(2,3,6,19:21)) %>% 
  filter(treatment != "Cage",
         treatment != "Cage/Open", # remove Cage treatments
         final_weight_g != "No measurements") %>% # remove corals with no final measurements 
  mutate(alive_dead = if_else(percent_survival > 5, 1, 0), # Gives value indicating if the coral is alive or dead
...
```{r Checks for experimental biases}
...
#### Growth analyses for surviving corals
...
# Check for normality in different transformations
...
# Mixed effects models for volume change and mass change ~ structure treatment for immediate recruitment experiment

vol_change.year1.glmm <- glmmTMB(volume_change.prop.sqrt ~ treatment,
                                       data = recruits.trial_1)
Anova(vol_change.year1.glmm)
summary(vol_change.year1.glmm)

mass_change.year1.glmm <- glmmTMB(mass_change.prop.sqrt ~ treatment,
                                       data = recruits.trial_1)
...
vol_change.year2.glmm <- glmmTMB(volume_change.prop.sqrt ~ treatment,
                                       data = recruits.trial_2)
Anova(vol_change.year2.glmm)

mass_change.year2.glmm <- glmmTMB(mass_change.prop.sqrt ~ treatment,
                                       data = recruits.trial_2)
...
#### Mortality analyses for recruits that did not survive
...
# Add in vector for numbers of replicates that died
...
# Calculate proportion of dead corals in each mortality type
