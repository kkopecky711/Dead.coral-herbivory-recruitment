...
algae_totals.short <- algae_totals %>% 
  ...
  mutate(days_since_start = as.factor(days_since_start),
         prop_const = total_macroalgae.prop + 0.001,
         log_trans.prop = log(prop_const))

hist(algae_totals.short$log_trans.prop)
...
mcr_coral.quad_totals <- mcr_coral %>% 
  group_by(date, location, cover_type) %>% 
  summarize(date = first(date),
            perc_cover = sum(percent_cover))

# Create all possible combinations
all_combinations <- expand.grid(
  date = unique(mcr_coral.quad_totals$date),
  location = unique(mcr_coral.quad_totals$location),
  cover_type = unique(mcr_coral.quad_totals$cover_type))

# Merge with original data and fill missing values with 0
mcr_coral.quad_totals.complete <- all_combinations %>%
  left_join(mcr_coral.quad_totals, by = c("date", "location", "cover_type")) %>%
  mutate(perc_cover = replace_na(perc_cover, 0))

mcr_coral.quad_totals.counts <- mcr_coral.quad_totals.complete %>% 
  group_by(date, cover_type) %>% 
  summarize(reps = n())

# Calculate means for each year
mcr_coral.summary <- mcr_coral.quad_totals.complete %>% 
  group_by(date, cover_type) %>% 
...
mcr_coral.quad_totals.2023 <- mcr_coral.2023 %>% 
  group_by(year, location, cover_type) %>% 
  summarize(#date = first(date),
            perc_cover = sum(percent_cover))
...
write_csv(mcr_coral.summary, "mcr.coral_algae_means.2005-2023.csv")
...
# Read in and clean MCR algae data
...
# Summary table of number of survivors to check if patch reef can be specified as a random effect
recruits.trial_1.summary <- recruits.trial_1 %>% 
  group_by(bommie_number) %>% 
  summarize(num_survivors = n())
